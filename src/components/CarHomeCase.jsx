...
function Hotspot({ hotspot, index }) {
  const [open, setOpen] = useState(false)
  const [tipPos, setTipPos] = useState(null)
  const btnRef = useRef(null)
  const tipRef = useRef(null)
  const closeTimer = useRef(null)
  const tipId = `hotspot-tip-${index}`
  ...
  // Flip above if tooltip clips the bottom — runs before browser paint
  useLayoutEffect(() => {
  ...
  return (
    <>
      <button
        ref={btnRef}
        className="absolute z-10 appearance-none bg-transparent border-0 flex items-center justify-center min-w-[44px] min-h-[44px] rounded-full focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2"
      ...
      >
        ...
        <span className="relative z-10 flex items-center justify-center w-5 h-5 rounded-full bg-blue-500" aria-hidden="true" />
      </button>
...
function MobileCarousel() {
  ...
  function handleKeyDown(e) {
    ...
    if (!el) return
    if (e.key === 'ArrowRight') { e.preventDefault(); el.scrollBy({ left: 316, behavior: 'smooth' }) }
    if (e.key === 'ArrowLeft')  { e.preventDefault(); el.scrollBy({ left: -316, behavior: 'smooth' }) }
  }
  ...
  return (
    <div className="relative">
      <div
        ref={scrollRef}
        role="region"
        aria-label="Mobile screen gallery — use arrow keys to navigate"
        tabIndex={0}
        onKeyDown={handleKeyDown}
        className="flex gap-6 overflow-x-auto snap-x snap-mandatory py-8 scrollbar-hide focus:outline-none"
      >
              ...
              <div className="phone-screen">
                <div className="relative">
                  ...
                  {mobileHotspots
                    ...
                    .map((hotspot, hi) => (
                      <Hotspot key={hi} hotspot={hotspot} index={`${i}-${hi}`} />
                    ))}
                </div>
              </div>
            </div>

            <p className="mt-4 text-xs font-semibold text-slate-500 tracking-widest uppercase" aria-hidden="true">
      ...
      <div className="lg:sticky lg:top-28">
        <div aria-live="polite" aria-atomic="true">
          <div className="flex items-center gap-3 mb-6">
            ...
            <span className="text-xs text-slate-500" aria-label={`Screen ${activeIndex + 1} of ${desktopScreens.length}`}>
              {activeIndex + 1} of {desktopScreens.length}
            </span>
          </div>
        ...
        {/* Progress dots */}
        <div className="flex items-center gap-1 mt-8" role="group" aria-label="Desktop screen navigation">
          {desktopScreens.map((_, i) => (
            <button
              ...
              className="min-w-[44px] min-h-[44px] flex items-center justify-center rounded-full focus-visible:ring-2 focus-visible:ring-blue-500"
            >
...
export default function CarHomeCase() {
  useEffect(() => {
    document.title = 'Car & Home Insurance | Dave Greene Portfolio'
    return () => { document.title = 'Dave Greene Portfolio' }
  }, [])
  ...
  return (
    <div className="bg-white min-h-screen" style={{ fontFamily: 'Inter, system-ui, sans-serif' }}>
      ...
      {/* Nav back */}
      <nav aria-label="Back to portfolio" className="border-b border-slate-100">
      ...
      <header className="pt-10 pb-6 border-b border-slate-100">
        <div className="max-w-7xl mx-auto px-4">
          <p className="text-xs font-semibold text-blue-600 tracking-widest uppercase mb-4" aria-hidden="true">Case Study</p>

          <h1 className="text-5xl font-bold text-slate-900 leading-tight mb-4">
            Inception to go-live in six months
          </h1>

          <ul className="flex flex-wrap gap-2 mt-5 list-none" aria-label="Project tags">
            {['Car insurance', 'House insurance', 'White labelling', 'Digital transformation', 'Responsive design', 'B2C'].map(tag => (
              <li key={tag} className="px-3 py-1 bg-slate-100 text-slate-600 text-sm rounded-full font-medium">
                {tag}
              </li>
            ))}
          </ul>
      ...
      <main>
        {/* Section A — Desktop journey */}
        <section className="pt-8 pb-12 bg-white" aria-labelledby="section-a-heading">
          <div className="max-w-7xl mx-auto px-4">
            <h2 id="section-a-heading" className="sr-only">01. Desktop Journey</h2>
            <div className="mb-16">
        ...
        {/* Section B — Mobile onboarding */}
        <section className="py-12" aria-labelledby="section-b-heading">
          <div className="max-w-7xl mx-auto px-4">
            <div className="mb-8">
              <p className="text-xs font-semibold text-blue-600 tracking-widest uppercase mb-3" aria-hidden="true">Section B</p>
              <h2 id="section-b-heading" className="text-4xl font-bold text-slate-900 mb-4">02. Frictionless Onboarding</h2>
